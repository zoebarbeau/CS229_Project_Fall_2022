...
from utilities.plot_triangles_kmeans import *
from utilities.plot_triangles_clusters import *
...
DS_name = 'TESTCS229'
trail = '_kmeans_FRF_cluster3_PH1p2_0p5_0p8_layer3dense_PCA_Norm01'
...
for f in feature_set.keys(): # LOOP OVER FEATURE SETS 
    ...
    for n in norms:# LOOP OVER NORMS SETS 
        ...
        if PCA:
            features_max, numpca = pca_features(features_train_nc,kmeans_path)
            features_test_max = pca_features_test(features_test_nc,numpca,kmeans_path)
        else:
            features_max = features_train_nc
            features_test_max = features_test_nc
        #Use kmeans
        kmeans = KMeans(n_clusters=numcluster).fit(features_max)
        prediction = kmeans.predict(features_test_max)          
        ...
        for case in cases_test:
            ...
            features_test2      = np.array(df[features])
            if PCA:
              features_test_max = pca_features_test(features_test2,numpca,kmeans_path)
            prediction = kmeans.predict(features_test_max)
            # get org RANS data
            filename = case + '_'
        ...
        for i in range(numcluster):
            features_train = features_max[kmeans.labels_ == i, :]
            labels_train  = labels_train_nc[kmeans.labels_ == i, :]
            thrss_train   = thrss_train_nc[kmeans.labels_ == i, :]
            features_test = features_test_max[prediction == i, :]
...
plot_triangle_kmeans(model, labels, feature_set, norms, 
                  DS_name, output_main, kmeans_file,numcluster,coef_v,numpca,PCA,cases, cols,colorstyle,cmap_list,SW_emphasis)


plot_triangle_clusters(model, labels, feature_set, norms, 
                 DS_name, output_main, kmeans_file,numcluster,coef_v,numpca,PCA,cases, cols,colorstyle,cmap_list,SW_emphasis)                